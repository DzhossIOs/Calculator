...
open class Attribute {
     ...
     */
    public init() {
        constant = Constant(value: 0.0, relation: .equal, multiplier: 1.0)
        priority = .required
    }
    ...
    public init(_ value: CGFloat) {
        constant = Constant(value: value, relation: .equal, multiplier: 1.0)
        priority = .required
    }
    ...
    @discardableResult func createConstraints(for item: Item) -> [NSLayoutConstraint] {
        guard ownedByItem || item.owningView != nil else {
        ...
        let constantFactor: CGFloat = createAttribute.shouldInvertConstant ? -1 : 1
        let layoutConstraint = NSLayoutConstraint(
            item: item,
            attribute: createAttribute.layoutAttribute,
            relatedBy: constant.relation,
            toItem: referenceItem,
            attribute: referenceAttributeHelper().layoutAttribute,
            multiplier: constant.multiplier,
            constant: constant.value * constantFactor
        )

        // Set priority
        #if swift(>=4.0)
            #if os(iOS) || os(tvOS)
                layoutConstraint.priority = UILayoutPriority(rawValue: priority.layoutPriority())
            #else
                layoutConstraint.priority = NSLayoutConstraint.Priority(rawValue: priority.layoutPriority())
            #endif
        #else
            layoutConstraint.priority = priority.layoutPriority()
        #endif
    ...
    func shouldInstall() -> Bool {
        ...
        #if os(iOS)
            let item = createItem?.owningView ?? createItem
            if let contextualCondition = condition as? ContextualCondition, let view = item as? View {
    ...
    func referenceAttributeHelper() -> ReferenceAttribute {
        ...
        if let attribute = referenceAttribute {
            return attribute
        }

        // If reference view is the superview then return same attribute
        // as `createAttribute`
        if let referenceItem = self.referenceItem, referenceItem === createItem?.owningView {
            return createAttribute
        }

        // Otherwise return the opposite of `createAttribute`
        return createAttribute.opposite
    }
}
