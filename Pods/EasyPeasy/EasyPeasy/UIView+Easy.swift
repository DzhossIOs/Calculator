...
// SOFTWARE.
     ...
     */
    private let traitCollectionDidChangeSwizzling: (UIView.Type) -> Void = { view in
    ...
    extension UIView {
        ...
        func easy_traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
            easy_traitCollectionDidChange(previousTraitCollection)

            // If at least one `Attribute has been applied to the current
            // `UIView` then perform `easy_reload`
            if traitCollection.containsTraits(in: previousTraitCollection) == false, nodes.values.count > 0 {
                easy.reload()
            }
        }
    }
